    ...
    <div className="space-y-5">
      ...
      <section className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        <div className="status-tile">
          ...
          <p className="mt-2 text-base font-semibold text-slate-950">{participantCards.length}</p>
          <p className="mt-1 text-xs text-slate-500">회의에 실제 참여한 역할 수</p>
        </div>
        <div className="status-tile">
          <p className="metric-label">웹 근거</p>
          <p className="mt-2 text-base font-semibold text-slate-950">{run.webSources.length}</p>
          <p className="mt-1 text-xs text-slate-500">리서치에 사용된 출처 수</p>
        </div>
        <div className="status-tile">
          <p className="metric-label">첨부 컨텍스트</p>
          <p className="mt-2 text-base font-semibold text-slate-950">{run.attachments.length}</p>
          <p className="mt-1 text-xs text-slate-500">회의에 주입된 참고 자료</p>
        </div>
        <div className="status-tile">
          <p className="metric-label">근거 신뢰도</p>
          <p className="mt-2 text-base font-semibold text-slate-950">{confidence}/100</p>
          <div className="mt-2 h-2 rounded-full bg-slate-200">
            <div className="h-2 rounded-full bg-sky-500" style={{ width: `${confidence}%` }} />
          </div>
        </div>
      </section>
      ...
      <section className="panel space-y-4">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">Participants</p>
      ...
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1.32fr)_360px]">
        <div className="space-y-5">
          <section className="panel space-y-4">
            <div className="flex flex-wrap items-center justify-between gap-2">
              <div>
                <p className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">Strategy Board</p>
                <h2 className="section-title">최종 산출물</h2>
              </div>
              <CopyButton text={run.deliverable?.content || ''} />
            </div>
            <p className="text-sm text-slate-500">{deliverableTypeLabel}</p>

            {structured ? (
              <div className="space-y-4">
                <div className="grid gap-3 md:grid-cols-3">
                  <div className="soft-panel">
                    <p className="text-xs font-semibold uppercase tracking-[0.14em] text-slate-400">Objective</p>
                    <p className="mt-2 text-sm leading-6 text-slate-700">{structured.objective}</p>
                  </div>
                  <div className="soft-panel">
                    <p className="text-xs font-semibold uppercase tracking-[0.14em] text-slate-400">Target</p>
                    <p className="mt-2 text-sm leading-6 text-slate-700">{structured.target}</p>
                  </div>
                  <div className="soft-panel">
                    <p className="text-xs font-semibold uppercase tracking-[0.14em] text-slate-400">Core Message</p>
                    <p className="mt-2 text-sm leading-6 text-slate-700">{structured.coreMessage}</p>
                  </div>
                </div>
                ...
                <div className="grid gap-3 md:grid-cols-2">
                  {(executiveSummary.length ? executiveSummary : ['요약 데이터 없음']).map((line, idx) => (
                    <div key={`${line}-${idx}`} className="list-card">
                      <p className="text-xs font-semibold uppercase tracking-[0.14em] text-slate-400">Summary {idx + 1}</p>
                ...
                <div className="grid gap-4 xl:grid-cols-[1.02fr_0.98fr]">
                  <div className="soft-panel">
                    <div className="flex items-center justify-between gap-2">
                      <p className="text-sm font-semibold text-slate-950">채널 믹스</p>
                      ...
                      {structured.channelPlan.map((row, idx) => (
                        <div key={`${row.channel}-${row.format}-${idx}`}>
                          ...
                          <div className="h-2 rounded-full bg-slate-200">
                            <div className="h-2 rounded-full bg-sky-500" style={{ width: `${Math.max(0, Math.min(100, row.budgetPct))}%` }} />
                          </div>
                  ...
                  <div className="soft-panel">
                    <div className="flex items-center justify-between gap-2">
                      <p className="text-sm font-semibold text-slate-950">KPI 스냅샷</p>
                    ...
                    <div className="mt-3 grid gap-3">
                      {structured.kpiTable.map((row, idx) => (
                        <div key={`${row.kpi}-${idx}`} className="list-card">
                          <div className="flex items-center justify-between gap-2">
                            <p className="text-sm font-semibold text-slate-900">{row.kpi}</p>
                            <span className="pill-option">{row.period}</span>
                          </div>
                          <div className="mt-3 grid gap-2 sm:grid-cols-2">
                            <div className="soft-card">
                              <p className="text-xs text-slate-400">Baseline</p>
                              <p className="mt-1 text-sm font-semibold text-slate-800">{row.baseline}</p>
                            </div>
                            <div className="soft-card">
                              <p className="text-xs text-slate-400">Target</p>
                              <p className="mt-1 text-sm font-semibold text-slate-800">{row.target}</p>
                            </div>
                          </div>
                        </div>
                      ))}
                ...
                <div className="grid gap-4 xl:grid-cols-[1.02fr_0.98fr]">
                  <div className="soft-panel">
                    <div className="flex items-center justify-between gap-2">
                      <p className="text-sm font-semibold text-slate-950">실행 타임라인</p>
                  ...
                  <div className="soft-panel">
                    <div className="flex items-center justify-between gap-2">
                      <p className="text-sm font-semibold text-slate-950">즉시 액션</p>
                ...
                <div className="grid gap-4 xl:grid-cols-[0.98fr_1.02fr]">
                  <div className="soft-panel">
                    <div className="flex items-center justify-between gap-2">
                      <p className="text-sm font-semibold text-slate-950">리스크 매트릭스</p>
                    ...
                    <div className="mt-3 space-y-3">
                      {structured.riskMatrix.map((row, idx) => (
                        <div key={`${row.risk}-${idx}`} className="list-card">
                          ...
                          <p className="mt-2 text-sm leading-6 text-slate-700">{row.mitigation}</p>
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  <div className="soft-panel">
                    <div className="flex items-center justify-between gap-2">
                      <p className="text-sm font-semibold text-slate-950">회의 로그</p>
                        ...
                        return (
                          <div key={turn.id} className="list-card">
                            <div className="flex flex-wrap items-center gap-2">
                              <p className="text-sm font-semibold text-slate-900">{parsed.baseName}</p>
                              {parsed.phaseLabel && <span className="pill-option">{parsed.phaseLabel}</span>}
                              <span className="rounded-full border border-slate-200 bg-slate-50 px-2.5 py-1 text-[11px] font-semibold text-slate-500">
                                {parsed.roleLabel}
                              </span>
                            </div>
                            <p className="mt-2 text-xs text-slate-500">{formatDate(turn.createdAt)}</p>
                            <pre className="mt-3 whitespace-pre-wrap text-sm leading-6 text-slate-700">{turn.content}</pre>
                          </div>
                        );
                      })}
                    </div>
              ...
              <pre className="whitespace-pre-wrap rounded-[22px] border border-slate-200 bg-slate-50 p-4 text-sm leading-7 text-slate-700">
          ...
          <section className="panel space-y-4">
            ...
            {run.attachments.length === 0 ? (
              <div className="soft-panel">
                <p className="text-sm text-slate-600">첨부된 자료가 없습니다.</p>
              </div>
            ) : (
              <div className="grid gap-3">
                {run.attachments.map((attachment, idx) => (
                  <div key={attachment.id} className="list-card">
                    <div className="flex flex-wrap items-center justify-between gap-2">
                      <div>
                        <p className="text-xs font-semibold uppercase tracking-[0.14em] text-slate-400">Attachment {idx + 1}</p>
                        <p className="mt-2 text-sm font-semibold text-slate-900">{attachment.name}</p>
                      </div>
                      <span className="pill-option">{attachment.mimeType}</span>
                    </div>
                    <p className="mt-2 text-xs text-slate-500">{formatDate(attachment.createdAt)}</p>
                    <pre className="mt-3 max-h-44 overflow-auto whitespace-pre-wrap rounded-[18px] border border-slate-200 bg-slate-50 p-3 text-xs leading-6 text-slate-700">
                      {attachment.content}
                    </pre>
                  </div>
                ))}
          ...
          <section className="panel space-y-4">
            <div className="flex flex-wrap items-center justify-between gap-2">
              <div>
                <p className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">Learning Matches</p>
            ...
            {(run.relatedLearnings || []).length === 0 ? (
              <div className="soft-panel">
                <p className="text-sm text-slate-600">추천 가능한 학습 카드가 없습니다. `/learning`에서 동기화를 먼저 실행하세요.</p>
              </div>
            ) : (
              <div className="grid gap-3 md:grid-cols-2">
                {(run.relatedLearnings || []).map((item) => (
                  <article key={item.id} className="list-card">
                    <div className="flex items-center justify-between gap-2">
                      <p className="text-xs font-semibold uppercase tracking-[0.14em] text-slate-400">Learning Card</p>
                      <span className={`rounded-full px-2.5 py-1 text-[11px] font-semibold ${learningTone(item.status)}`}>
                        {item.status}
                      </span>
                    </div>
                    <p className="mt-3 text-sm font-semibold leading-6 text-slate-900">{item.situation}</p>
        ...
        <aside className="space-y-5 xl:sticky xl:top-24 xl:self-start">
          <section className="panel space-y-4">
            <div className="flex items-center justify-between gap-2">
              <div>
                <p className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">PM Decision</p>
            ...
            <div className="grid gap-3">
              {(pmHighlights.length ? pmHighlights : ['PM 결정이 없습니다.']).map((line, idx) => (
                <div key={`${line}-${idx}`} className="list-card">
                  <p className="text-xs font-semibold uppercase tracking-[0.14em] text-slate-400">Decision {idx + 1}</p>
                  <p className="mt-2 text-sm leading-6 text-slate-700">{line}</p>
                </div>
              ))}
            </div>
          ...
          <section className="panel space-y-4">
            <div className="flex items-center justify-between gap-2">
              <div>
                <p className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">Evidence Rail</p>
            ...
            <div className="space-y-2">
              {run.webSources.length === 0 ? (
                <div className="soft-panel">
                  <p className="text-sm text-slate-600">웹 소스가 없습니다.</p>
                </div>
              ) : (
                run.webSources.slice(0, 5).map((source, idx) => (
                  <a key={source.id} href={source.url} target="_blank" rel="noreferrer" className="list-card block">
                    <p className="text-xs font-semibold uppercase tracking-[0.14em] text-slate-400">Source {idx + 1}</p>
                    <p className="mt-2 text-sm font-semibold leading-6 text-slate-900">{source.title}</p>
                    <p className="mt-2 line-clamp-3 text-xs leading-5 text-slate-500">{source.snippet}</p>
                  </a>
                ))
              )}
            </div>
          ...
          <section className="panel space-y-4">
            <div className="flex items-center justify-between gap-2">
              <div>
                <p className="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">Memory Cockpit</p>
            ...
            <div className="grid gap-3">
              <div className="soft-panel">
                <p className="text-xs font-semibold uppercase tracking-[0.14em] text-slate-400">가설</p>
                <p className="mt-2 text-sm leading-6 text-slate-700">{run.memoryLog?.hypothesis || '없음'}</p>
              </div>
              <div className="soft-panel">
                <p className="text-xs font-semibold uppercase tracking-[0.14em] text-slate-400">전략 방향</p>
                <p className="mt-2 text-sm leading-6 text-slate-700">{run.memoryLog?.direction || '없음'}</p>
              </div>
              <div className="soft-panel">
                <p className="text-xs font-semibold uppercase tracking-[0.14em] text-slate-400">예상 KPI 영향</p>
                <p className="mt-2 text-sm leading-6 text-slate-700">{run.memoryLog?.expectedImpact || '없음'}</p>
              </div>
              <div className="soft-panel">
                <p className="text-xs font-semibold uppercase tracking-[0.14em] text-slate-400">리스크</p>
                <p className="mt-2 text-sm leading-6 text-slate-700">{run.memoryLog?.risks || '없음'}</p>
              </div>
            </div>
            ...
            {structured && (
              <div className="soft-card">
                <div className="flex items-center justify-between gap-2">
                  <p className="text-sm font-semibold text-slate-950">근거 및 가정</p>
                  <span className="pill-option">{confidence}/100</span>
                </div>
                <p className="mt-3 text-xs text-slate-400">Source IDs</p>
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium text-slate-900">실제 성과 피드백</label>
              <textarea
                value={outcomeInput}
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium text-slate-900">실패 원인 / 개선 포인트</label>
              <textarea
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium text-slate-900">태그</label>
