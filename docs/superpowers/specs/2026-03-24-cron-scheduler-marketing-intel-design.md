...
### Phase 5: UX/UI 고도화 — 고급 (에이전트 자율성)

30. AI 코파일럿 사이드바 (`Cmd+.` 토글, 컨텍스트 인식 채팅)
31. 승인 인박스 (에이전트 자율 작업 → 인간 판단 요청 큐)
32. 세션 리플레이 (에이전트 실행 단계별 재생, LLM 호출/비용 추적)
33. 에이전트 메모리 사이드바 (참조 컨텍스트 표시, 사용자 편집 가능)
34. 워치리스트 관리 UI (키워드 추가/삭제/카테고리/플랫폼)
35. 마케팅 인텔 대시보드 (피드 뷰, 플랫폼 필터, 관련도 정렬)
36. 다이제스트 뷰어 (타임라인, 인사이트 카드)
37. 수집기 모니터링 (API 키 상태, 쿼터 현황, 에러)
