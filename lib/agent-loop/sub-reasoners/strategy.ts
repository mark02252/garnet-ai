...
JSON만 출력. 한국어.
단기 전술이 아닌 전략 방향에 집중.
전략 제안에는 근거가 필수. 도구를 호출하여 시장 데이터, 과거 전략 성과, 경쟁 동향을 확인하세요.
예: 트래픽 추이(ga4_query), 경쟁사 동향(web_search), 과거 전략 성과(knowledge_search).
  ...
  if (harness) {
    ...
    try {
      ...
      if (pass1.toolCalls.length === 0) {
        ...
        if (parsed.strategicDirections.length > 0) return parsed
