    ...
    return (
      <div className="space-y-4">
        ...
        <section className="panel space-y-3">
          ...
          <pre className="whitespace-pre-wrap rounded-[22px] border border-slate-200 bg-white px-4 py-4 text-sm leading-7 text-slate-700">
    ...
    <div className="space-y-5">
      ...
      <section className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        <div className="status-tile">
          ...
          <p className="mt-2 text-base font-semibold text-slate-950">{primaryChannel?.channel || '미정'}</p>
          <p className="mt-1 text-xs text-slate-500">{primaryChannel ? `${primaryChannel.budgetPct}% · ${primaryChannel.format}` : '채널 계획 없음'}</p>
        </div>
        <div className="status-tile">
          <p className="metric-label">예산 커버리지</p>
          <p className="mt-2 text-base font-semibold text-slate-950">{totalBudget}%</p>
          <p className="mt-1 text-xs text-slate-500">채널 플랜 합산 기준</p>
        </div>
        <div className="status-tile">
          <p className="metric-label">대표 KPI</p>
          <p className="mt-2 text-base font-semibold text-slate-950">{topKpi?.kpi || '미정'}</p>
          <p className="mt-1 text-xs text-slate-500">{topKpi ? `${topKpi.target} · ${topKpi.period}` : 'KPI 없음'}</p>
        </div>
        <div className="status-tile">
            ...
            {structured.channelPlan.map((row, idx) => (
              <div key={`${row.channel}-${idx}`} className="soft-panel">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <p className="text-sm font-semibold text-slate-950">{row.channel}</p>
                    <p className="mt-1 text-xs text-slate-500">{row.format} · KPI {row.kpi}</p>
                  </div>
                ...
                <p className="mt-3 text-sm text-slate-700">목표: {row.targetValue}</p>
              </div>
            ))}
          </div>
        </div>
            ...
            {structured.kpiTable.map((row, idx) => (
              <div key={`${row.kpi}-${idx}`} className="list-card">
                <div className="flex items-center justify-between gap-2">
                  <p className="text-sm font-semibold text-slate-950">{row.kpi}</p>
          ...
          <div className="space-y-3">
            {structured.timeline.map((row, idx) => (
              <div key={`${row.phase}-${idx}`} className="list-card">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <p className="text-sm font-semibold text-slate-950">{row.phase}</p>
                  <span className="pill-option">
                    {row.start} - {row.end}
                  </span>
                </div>
                <p className="mt-2 text-xs text-slate-500">Owner · {row.owner}</p>
          ...
          <div className="space-y-3">
            {structured.riskMatrix.map((row, idx) => (
              <div key={`${row.risk}-${idx}`} className="list-card">
                <p className="text-sm font-semibold text-slate-950">{row.risk}</p>
                ...
                <p className="mt-3 text-sm leading-6 text-slate-700">대응: {row.mitigation}</p>
              </div>
            ))}

            <div className="soft-panel">
              <p className="text-sm font-semibold text-slate-950">근거 Source IDs</p>
            ...
            <div className="soft-panel">
              <p className="text-sm font-semibold text-slate-950">가정</p>
      ...
      <section className="panel space-y-3">
        ...
        <div className="soft-panel">
          <pre className="whitespace-pre-wrap text-sm leading-7 text-slate-700">{pmDecision || 'PM 결정이 없습니다.'}</pre>
        </div>
      </section>

      <details className="panel">
        <summary className="cursor-pointer text-sm font-semibold text-slate-800">원본 산출물 보기</summary>
