...
**LLM 호출:** `runLLM(systemPrompt, userPrompt, 0.4, 4000, { llmProvider: 'gemma4' })` — 기존 `runLLM` 인프라를 사용 (Gemini API 호출은 gemma4 프로바이더가 처리). 환경변수 기반 모델 ID 해석도 자동.

**에러 처리 / Retry 전략:**
1. JSON 파싱 실패 → 1회 재시도 (에러 메시지를 userPrompt에 첨부)
2. `validateFlow()` 실패 (사이클, 도달 불가 노드) → 끊어진 edge 자동 제거 후 재검증. 재검증도 실패 시 1회 재시도
3. 2회 재시도 후에도 실패 → "플로우 설계에 실패했습니다. 프로젝트를 더 구체적으로 설명해주세요." 응답
4. 총 최대 LLM 호출: 원본 1회 + 재시도 1회 = 2회
...
**`safeParseIntent()` 수정 (line ~73):** flow 타입을 인식하도록 파싱 분기 추가:

```typescript
if (action.type === 'flow-create') return { action: { type: 'flow-create', projectDescription: action.projectDescription || '' }, reasoning }
if (action.type === 'flow-run') return { action: { type: 'flow-run', userInput: action.userInput || '' }, reasoning }
if (action.type === 'flow-list') return { action: { type: 'flow-list' }, reasoning }
if (action.type === 'flow-converse') return { action: { type: 'flow-converse', question: action.question || '' }, reasoning }
```

**`keywordFallback()` 수정 (line ~95):** flow 키워드 패턴 추가:

```typescript
if (/플로우.*(만들|생성|구성|설계)/i.test(text)) return { action: { type: 'flow-create', projectDescription: text }, reasoning: 'keyword: flow-create' }
if (/플로우.*(실행|돌려|돌리|시작)/i.test(text)) return { action: { type: 'flow-run', userInput: text }, reasoning: 'keyword: flow-run' }
if (/플로우.*(목록|리스트|저장된|보여)/i.test(text)) return { action: { type: 'flow-list' }, reasoning: 'keyword: flow-list' }
```

### 2-4. Shell API 엔드포인트

**`/api/flow-templates/generate` 는 별도 API route를 만들지 않음** — `generateFlowBlueprint()`는 라이브러리 함수(`lib/flow/architect.ts`)로만 존재하며, `/api/agent/command` 내부에서 직접 호출한다. Shell SSE 스트림 안에서 blueprint를 생성하고 `flow-preview` 이벤트로 전송하므로 별도 엔드포인트 불필요.
...
```typescript
// 기존: { name: "새 플로우" } → 기본 노드
// 확장: { name: "카페 마케팅", nodes: "[JSON string]", edges: "[JSON string]" } → Architect가 생성한 노드로 저장
```

`nodes`와 `edges`는 Prisma 컬럼과 동일하게 **JSON 문자열**로 전달 (파싱된 객체가 아님).

### 2-5. Canvas 패널 타입 확장 (`lib/canvas-store.ts`)

`PanelData` 유니온에 `flow-preview` 변형 추가:

```typescript
// lib/canvas-store.ts 기존 PanelData 유니온에 추가:
...
export type PanelData =
  ...
  | { type: 'flow-preview';  data: FlowPreviewData }    // 추가
```

`TypedPanelContent` (canvas-panel.tsx line ~144)에 렌더링 분기 추가:

```typescript
case 'flow-preview':
  return <FlowPreviewPanel panel={panel} />
```

`FlowPreviewPanel`은 `useFlowRunStore`를 구독하여 실행 중 노드 상태를 실시간 반영.

### 2-6. Canvas 미니 플로우 다이어그램 (`FlowPreviewPanel`)
...
type FlowPreviewPanelProps = {
  panel: CanvasPanelType  // canvas-store의 패널 데이터 (data: FlowPreviewData)
}
```

**Zustand 연동:** 기존 `useFlowRunStore` (`lib/flow/run-store.ts`)를 재사용한다. Shell에서 실행하든 에디터에서 실행하든 동일한 스토어를 사용하므로 `nodeStatuses`, `isRunning`, `runId` 등을 그대로 구독. 별도 스토어를 만들지 않는다.
...
### 2-7. Command Bar 처리 흐름 확장

현재 `command-bar.tsx`는 `/api/agent/command`에 POST하고 SSE 이벤트를 처리함. **SSE 프로토콜:** 표준 SSE `event:` 필드가 아닌 **nested JSON** 방식 (`data: { "event": "...", "data": { ... } }`)을 사용. Flow 이벤트도 동일한 convention을 따름.

Flow 관련 이벤트 타입을 추가 — `handleSSEEvent` switch문 (line ~211)에 분기 추가:

```typescript
// 기존 SSE 이벤트 (nested JSON)
...
| 파일 | 변경 | 설명 |
|------|------|------|
| `lib/flow/architect.ts` | 신규 | Flow Architect — LLM 기반 플로우 자동 설계 + retry/repair |
| `lib/flow/template-matcher.ts` | 신규 | LLM 기반 템플릿 매칭 |
| `lib/flow/architect-prompt.ts` | 신규 | Architect용 systemPrompt + JSON 스키마 |
| `lib/agent-intent.ts` | 수정 | flow 인텐트 추가 + `safeParseIntent()` + `keywordFallback()` 수정 |
| `lib/canvas-store.ts` | 수정 | `PanelData`에 `flow-preview` 변형 + `FlowPreviewData` 타입 추가 |
| `app/api/flow-templates/route.ts` | 수정 | POST에 nodes/edges(JSON 문자열) 옵션 추가 |
| `app/api/agent/command/route.ts` | 수정 | flow intent 처리 + SSE 이벤트 추가 (nested JSON convention) |
| `components/agent-shell/flow-preview-panel.tsx` | 신규 | 미니 플로우 다이어그램 + 실행 상태 (useFlowRunStore 재사용) |
| `components/agent-shell/canvas-panel.tsx` | 수정 | `TypedPanelContent`에 flow-preview case 추가 |
| `components/agent-shell/command-bar.tsx` | 수정 | `handleSSEEvent`에 flow 이벤트 5개 분기 추가 |
