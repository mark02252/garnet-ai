...
export async function fetchMovieRevenueTop(days = 7, limit = 10): Promise<GA4MovieRevenue[]> {
  ...
  const endDate = days <= 1 ? 'yesterday' : 'today';
  const [response] = await client.runReport({
    property: `properties/${creds.propertyId}`,
    dateRanges: [{ startDate, endDate }],
...
export async function fetchTheaterRevenueTop(days = 7, limit = 10): Promise<GA4TheaterRevenue[]> {
  ...
  try {
    ...
    // 같은 이름으로 매핑된 지점 합산 (raw/encrypted/한글 코드 중복 방지)
    const merged = new Map<string, GA4TheaterRevenue>();
    for (const row of rawRows) {
      const key = row.theaterName || row.theaterCode;
      const existing = merged.get(key);
      if (existing) {
        existing.purchased += row.purchased;
...
반드시 아래 JSON 형식으로만 출력하세요:
{
  ...
  // 트래픽 요약 (최근 7일만 — 프롬프트 크기 최적화)
  const trafficFull = completedTraffic.slice(-7).map(d => ({
...
5. 이상 징후가 있다면 원인 추정과 대응 방안`;
  ...
  let raw = ''
  try {
    // runtime이 없으면 명시적으로 gemini 사용 (OpenClaw 폴백 방지)
    const effectiveRuntime = runtime || { llmProvider: 'gemini' } as RuntimeConfig
    raw = await runLLM(systemPrompt, userPrompt, 0.3, 4000, effectiveRuntime);
  } catch (llmErr) {
    console.error('[analyzeGA4WithAI] LLM 호출 실패:', llmErr instanceof Error ? llmErr.message : llmErr);
    return {
      summary: 'AI 분석 호출에 실패했습니다. LLM 쿼터를 확인하세요.',
      highlights: [], recommendations: [], anomalies: [], generatedAt: new Date()
    };
  }
